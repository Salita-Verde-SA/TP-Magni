...
export function ParticipantesProvider({ children }: { children: React.ReactNode }) {
  ...
  // Crea participante en backend y despacha al reducer.
  ...
  // Actualiza participante en backend y despacha al reducer.
  const editar = useCallback(async (modificado: Participante) => {
    ...
    setParticipanteEnEdicion(null) // Limpiar seleccion tras editar con exito
  }, [])

  // Elimina participante por id en backend y despacha al reducer.
  ...
  // Borra todos los participantes en backend y despacha al reducer.
